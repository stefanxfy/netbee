...
func ShowSimpleHelp() {
	...
	fmt.Println("  -keep")
	fmt.Println("        继续使用上次 AI 诊断的 conversation_id")
...
func ShowHelp() {
	...
	fmt.Println()
	fmt.Println("  # 基于上次会话继续追问")
	fmt.Println("  sudo NETBEE_AI_API_KEY=your-token ./target/netbee -c 50 -AI -keep -ai-out diagnosis.md")
	fmt.Println()
}
